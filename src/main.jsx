...
function CardScanner(){
 ...
 async function startCamera(){setError("");setBusy(true);try{if(!navigator.mediaDevices?.getUserMedia)throw new Error("Camera access is not available in this browser.");const stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:"environment"},width:{ideal:1280},height:{ideal:720}},audio:false});streamRef.current=stream;setCameraOn(true);requestAnimationFrame(()=>{if(videoRef.current){videoRef.current.srcObject=stream;videoRef.current.play().catch(()=>{})}})}catch(e){setError(e?.name==="NotAllowedError"?"Camera permission was denied. Allow camera access and try again.":e?.message||"Could not open the camera.");setCameraOn(false)}finally{setBusy(false)}}
 function stopCamera(){if(streamRef.current){streamRef.current.getTracks().forEach(t=>t.stop());streamRef.current=null}if(videoRef.current)videoRef.current.srcObject=null;setCameraOn(false)}
 function capture(){const v=videoRef.current;if(!v||v.readyState<2)return;const c=document.createElement("canvas");c.width=v.videoWidth;c.height=v.videoHeight;c.getContext("2d").drawImage(v,0,0,c.width,c.height);setShot(c.toDataURL("image/jpeg",.9))}
 useEffect(()=>()=>stopCamera(),[]);
 return <div className="panel"><div className="heading" style={{marginTop:0}}><div><span className="eyebrow">CARD SCANNER</span><h2>Scan a card</h2><p>First milestone: capture a Pokémon card with your phone camera. Identification comes next.</p></div></div>{error&&<div className="alert">{error}</div>}<div style={{display:"grid",gridTemplateColumns:"minmax(0,1.4fr) minmax(260px,1fr)",gap:16,alignItems:"start"}}><div><div style={{position:"relative",width:"100%",aspectRatio:"16/10",background:"#111827",borderRadius:14,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center"}}>{cameraOn?<video ref={videoRef} playsInline muted autoPlay style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{color:"#fff",padding:24,textAlign:"center"}}><b>Camera is off</b><small style={{display:"block",marginTop:8,opacity:.75}}>Use your phone's rear camera and point it at one card.</small></div>}{cameraOn&&<div style={{position:"absolute",inset:"10% 18%",border:"2px solid rgba(255,255,255,.8)",borderRadius:12,pointerEvents:"none"}}/>}</div><div className="modalactions" style={{marginTop:12}}>{!cameraOn?<button onClick={startCamera} disabled={busy}>{busy?"Opening camera…":"Open camera"}</button>:<><button onClick={capture}>Capture card</button><button className="ghost" onClick={stopCamera}>Stop camera</button></>}</div></div><div><span className="eyebrow">CAPTURE</span>{shot?<div style={{marginTop:8}}><img src={shot} alt="Captured Pokémon card" style={{width:"100%",borderRadius:12,display:"block"}}/><button className="ghost" style={{marginTop:10}} onClick={()=>setShot(null)}>Clear capture</button></div>:<div className="empty" style={{marginTop:8}}><b>📷</b><h3>No card captured</h3><p>Capture a card to test the camera workflow.</p></div>}</div></div></div>
}
